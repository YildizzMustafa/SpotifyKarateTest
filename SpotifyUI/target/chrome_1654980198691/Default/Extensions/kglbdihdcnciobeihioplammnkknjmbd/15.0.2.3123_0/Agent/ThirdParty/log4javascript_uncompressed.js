 ...
 *                 200131
 * Modify the code to comply with Firefox add-on hub review.
 *  1. comment out log4javascript.evalInScope as 'eval' is not allowed by Firefox add-on hub.
 *  2. re-write setTimeout(sendAll, timerInterval); to setTimeout(function() { sendAll(); }, timerInterval);
 *
 * Changed By: Niv Lederer.
 * Office Address: Mercury Interactive (Israel) Limited
 *				   Shabazi st. No 26 
 *				   Yehud 
 *				   Israel 
 *				   56100
 * In Firefox addon there are cases where opera is defined on global namespace but the opera object is in accessible 
 *  due to security restrictions. Treat this case as if opera is not defined.
 */
...
/* ---------------------------------------------------------------------- */
	...
	function AjaxAppender(url) {
		...
		function scheduleSending() {
		    setTimeout(function() { sendAll(); }, timerInterval);
		}
